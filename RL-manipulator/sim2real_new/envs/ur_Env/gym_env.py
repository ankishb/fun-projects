...
class UrGymEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self._obs = dict()
        # self._obs = dict.fromkeys(['state','goal','joint'])   # edited according to ankish
        self._start_orient = np.random.uniform(-1,1,size=[10,3])
        self._goal_orient = np.random.uniform(-1,1,size=[10,3])

    def _set_env(self):
        # gym.make doesnt allow to pass arguments, this is kind of workanp.round for that
        if self._urdf_root is None:
            self._urdf_root = os.path.join("/home/mohit/ankish/sim2real_new/ur_description", '/home/mohit/ankish/sim2real_new/ur_description/urdf/ur10_robot.urdf')
        if self._renders:
...
#            self._real_robot.arm.go_home()
        ...
        self._obs['state'] = np.array(self._get_position())
        self._obs['goal'] = np.array(self._goal)
        self._obs['joint'] = np.array(self._get_joints())
...
#        return np.array(self._get_joints() + self._goal)
    ...
    def step(self, a, s):
        self._num_steps += 1
        # get the new_theta
        # print("action: ", np.round(a,2), "\n", "joint: ", np.round(self._get_joints(),2), "\n", "s", np.round(s,2), "\n")
        required_joints = np.clip(np.array(self._get_joints()) + a*0.7, -np.pi / 2, np.pi / 2)
        # required_joints = np.clip( a, -np.pi / 2, np.pi / 2)
        # print("new joint: ", np.round(np.array(self._get_joints()) + a,2))
        # print("new joint 2 : ", np.round(required_joints, 2))
        print()
...
#            required_joints = self._real_robot.arm.get_joint_angles()
        ...
        if dist < self._threshold:
            ...
            return self._obs,self._reaching_rew,True,{}
