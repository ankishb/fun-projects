...
for ii in range(max_ep):
    ...
    R_graph.append(R)
    R_graph_.append(R)
    store_reward.append(np.mean(np.array(R_graph)))

    #print(ii, R)
    saver.save(sess, './log/rl-ddpg-her',global_step=5000)
    if ii % 10000 ==0 :
    	file = './reward/reward_'+str(ii)
    	np.save(file+'.npy', store_reward)

    if ii == 5 or ii % 1000 == 0 or ii == 10 or ii == 20:
    	print("M running ...")
...
saver.save(sess, './log/rl-ddpg-her-final')
np.save('./reward/reward-final.npy',store_reward)
...
for ii in range(max_ep):
    ...
    for kk in range(200):
        ...
        s = s2
        R += r 
    infer_r.append(np.mean(R))

evv.close()
np.save("./reward/infer_reward.npy",np.array(infer_r))
plt.plot(infer_r)
